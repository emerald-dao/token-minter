...
<Section class="padding-top-small">
  <Container>
    <div class="main-grid">
      {#await checkItemInfo then info}
        <div class="image-wrapper left-top">
          <NftImage
            thumbnailURL={`https://nftstorage.link/ipfs/${info.metadata.image.cid}/${info.metadata.image.path}`}
            name={`${info.metadata.name} NFT`} />
        </div>
        <div class="collection-info-wrapper">
          <Stack direction="column" align="flex-start">
            ...
            {:else if !info.metadata.purchasers[serial]}
              <Button
                leftIcon="wallet"
                loading={$transactionInProgress}
                on:click={() =>
        ...
        <div class="left-bottom">
          <Stack direction="column" align="flex-start" gap="2rem">
            ...
            <table>
              ...
              {#each Object.entries(info.metadata.extra) as metadataArray}
                <tr>
